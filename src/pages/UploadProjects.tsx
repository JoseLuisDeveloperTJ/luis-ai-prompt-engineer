...
import { Upload, X, Github, Plus, Trash2, Image, Video, Images } from 'lucide-react';
...
export default function UploadProjects() {
  ...
  const handleGallerySelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) setGalleryFiles([...galleryFiles, ...Array.from(e.target.files)]);
  };

  const removeGallery = (i: number) => setGalleryFiles(galleryFiles.filter((_, idx) => idx !== i));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: proj, error } = await supabase.from('projects').insert({
      ...
      cover_url, cover_type: coverFile ? coverType : null,
      skills: skills.length > 0 ? skills : null, github_link: githubLink.trim() || null,
    }).select().single();

    if (error) { toast({ title: 'Error', description: error.message, variant: 'destructive' }); setUploading(false); return; }

    // Upload gallery images
    if (proj && galleryFiles.length > 0) {
      for (let i = 0; i < galleryFiles.length; i++) {
        const file = galleryFiles[i];
        const ext = file.name.split('.').pop();
        const path = `${user.id}/gallery/${proj.id}/${Date.now()}_${i}.${ext}`;
        const { error: gError } = await supabase.storage.from('project-covers').upload(path, file);
        if (!gError) {
          const { data: gUrl } = supabase.storage.from('project-covers').getPublicUrl(path);
          await supabase.from('project_images').insert({
            project_id: proj.id, image_url: gUrl.publicUrl, sort_order: i,
          });
        }
      }
    }
    ...
    setName(''); setDescription(''); setSkills([]); setGithubLink('');
  ...
  const handleDelete = async (id: string) => {
    ...
    else { queryClient.invalidateQueries({ queryKey: ['projects'] }); toast({ title: 'Project deleted' }); }
  };
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="p-6 rounded-xl card-gradient border border-border/50 space-y-5">
        <div>
          ...
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="My AI Project"
            required maxLength={200} className="bg-background/50 border-border/50 focus:border-primary" />
        </div>
        ...
        <div>
          ...
          <Input type="file" accept={coverType === 'image' ? 'image/*' : 'video/*'}
            onChange={(e) => setCoverFile(e.target.files?.[0] ?? null)} className="bg-background/50 border-border/50" />
          {coverFile && (
            <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
              <span>{coverFile.name}</span>
              <button type="button" onClick={() => setCoverFile(null)} className="text-destructive hover:text-destructive/80"><X className="h-4 w-4" /></button>
            </div>
          )}
        </div>
        ...
        <div>
          <label className="text-sm font-mono text-muted-foreground mb-1.5 block">
            <Images className="h-4 w-4 inline mr-1" /> Gallery Images
          </label>
          <Input type="file" accept="image/*" multiple onChange={handleGallerySelect} className="bg-background/50 border-border/50" />
          {galleryFiles.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-3">
              {galleryFiles.map((f, i) => (
                <div key={i} className="relative group">
                  <img src={URL.createObjectURL(f)} alt="" className="w-20 h-16 object-cover rounded-md border border-border/50" />
        ...
        <div>
          ...
          <Textarea value={description} onChange={(e) => setDescription(e.target.value)}
            placeholder="Describe your project..." maxLength={2000} rows={4}
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            <Input value={skillInput} onChange={(e) => setSkillInput(e.target.value)}
              placeholder="e.g. Python, PyTorch..." maxLength={50}
        ...
        <div>
          ...
          <div className="relative">
            ...
            <Input value={githubLink} onChange={(e) => setGithubLink(e.target.value)}
              placeholder="https://github.com/..." maxLength={500} className="pl-10 bg-background/50 border-border/50 focus:border-primary" />
          </div>
        </div>

        <Button type="submit" variant="hero" disabled={uploading || !name.trim()}>
          <Upload className="h-4 w-4 mr-2" /> {uploading ? 'Uploading...' : 'Publish Project'}
        </Button>
      </form>
            ...
            {projects.map((p) => (
              <div key={p.id} className="flex items-center justify-between p-4 rounded-xl card-gradient border border-border/50">
                <div className="flex items-center gap-4">
                  {p.cover_url && <img src={p.cover_url} alt={p.name} className="w-16 h-12 object-cover rounded-md" />}
                  <div>
